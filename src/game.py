...
from cards import Card, Deck, Mode, CardRanks
...
class Game:
    ...
    def reset_game(self):
        ...
        for player in self.players:
            player.hand.cards = []
        self.licitator.hand.cards = []
        self.deck.shuffle() # Může být volitelné, pokud se balíček používá až do vyčerpání.
    
    def remove_cards(self, player: Player):
        """Předá karty z ruky do talonu."""
        self.game_logic.move_to_talon(player)
        
    
    def licitating(self):
        # TODO na serveru bude potřeba přidat komu se bude co posílat
        print("Licitátor vybírá trumf.")
        print(self.licitator.show_first_seven())
    ...
    def play_stych(self, starting_player_index: int):
        ...
        return winner_index, winner_card
    
    def play(self):
        mode = self.game_logic.mode 
        if mode == Mode.HRA or mode == Mode.SEDMA:
            self.hra_sedma()
        elif mode == Mode.BETL or mode == Mode.DURCH:
            self.betl_durch()
        else:
            raise Exception("Není určen mód hry!")
    ...
    def game_result(self):
        ...
        if licitator_points > players_points:
            print(f"Vyhrál Licitátor v poměru {licitator_points} : {players_points}")
            self.result = True
            return
            
        print(f"Vyhrál/i hráč/i v poměru {players_points} : {licitator_points}")
        self.result = False
    ...
    def hra_sedma(self):
        ...
        while self.licitator.has_card_in_hand():
            winner_index, winner_card = self.play_stych(starting_player_index)
            starting_player_index = winner_index
                
        if self.game_logic.mode == Mode.SEDMA:
            if winner_index == self.licitator.number and winner_card.__eq__(Card(CardRanks.SEDM, self.game_logic.trumph)):
                print("Licitátor vyhrál sedmu!")
            else:
                print("Licitátor prohrál sedmu!")
